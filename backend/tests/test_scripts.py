    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if os.name == 'nt' and setup_bat.exists():
            ...
            try:
                ...
                assert (temp_path / "backend" / "storage").exists(), "Storage directory not created"
                assert (temp_path / "backend" / "logs").exists(), "Logs directory not created"
                assert (temp_path / "backend" / "storage" / "images").exists(), "Images directory not created"
                
                # Check that requirements.txt was created
                assert (temp_path / "backend" / "requirements.txt").exists(), "requirements.txt not created"
                
                # Check that .env was created
                assert (temp_path / "backend" / ".env").exists(), ".env not created"
                
                # Check that venv was created
                assert (temp_path / "backend" / ".venv").exists(), "Virtual environment not created"
        ...
        # Test the shell script on Unix-like systems
            ...
            try:
                ...
                assert (temp_path / "backend" / "logs").exists(), "Logs directory not created"
                assert (temp_path / "backend" / "storage" / "images").exists(), "Images directory not created"
                assert (temp_path / "frontend" / "public").exists(), "Public directory not created"
                assert (temp_path / "frontend" / "public" / "output").exists(), "Output directory not created"
                
                # Check that requirements.txt was created
                assert (temp_path / "backend" / "requirements.txt").exists(), "requirements.txt not created"
                
                # Check that .env.local was created
                assert (temp_path / "frontend" / ".env.local").exists(), ".env.local not created"
                assert (temp_path / "backend" / ".env").exists(), ".env not created"
